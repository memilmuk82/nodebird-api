...
저장소 관리자 또는 권한 있는 담당자는 다음 조치를 수행해야 합니다.

1. DB 자격 증명, 쿠키·세션 비밀값과 JWT 서명 값을 폐기·회전합니다. 세션 비밀값과 JWT 비밀값의 교체는 기존 세션과 토큰을 무효화합니다.
2. 데이터베이스 접근 기록·권한과 카카오 애플리케이션 설정을 감사하고, 실제 비밀값이 더 확인되면 함께 폐기·회전합니다.
...
Domain 클라이언트 비밀값은 별도로 사용 현황을 확인해야 합니다. 실제 서비스에서 사용된 값이라면 회전·폐기 수단을 먼저 구현한 뒤 기존 값을 교체하고, 클라이언트 이전과 접근 기록 감사를 수행해야 합니다.

원격 이력 변경과 실제 자격 증명 회전은 이 README 정비 범위에 포함되지 않습니다.

README 정비 전 Git 상태에서 로컬 `main` 브랜치는 `origin/main`보다 1개 커밋 앞서 있었으며, 이 작업에서는 원격 push를 수행하지 않았습니다.

## 포트폴리오 전환 검토

최종 분류는 **F. 수동 확인 필요**입니다.

- 기능적 잠재력: 세션 기반 개발자 UI, 도메인 등록, JWT 발급과 보호된 SNS 조회 API를 한 흐름에서 보여 주며 NodeBird·NodeCat과 연계되는 구조가 명확합니다.
- 공개 전 필수 확인: 추적된 DB·쿠키·세션·JWT 자격 증명을 폐기·회전하고 기존 세션과 JWT를 무효화해야 합니다. 데이터베이스 접근 기록·권한, 카카오 애플리케이션 설정과 API 클라이언트 사용 기록을 감사하고, 비밀값의 Git 추적 제거와 승인된 원격·이력 정리·재검사의 완료 상태를 권한 있는 담당자가 확인해야 합니다.

보안 조치 후에도 공개 전 다음 보완이 필요합니다.

- Domain 클라이언트 비밀값의 해시 저장, 마스킹 표시, 개별 회전·폐기와 마지막 사용 감사
- JWT의 기대 issuer, 허용 알고리즘, audience와 scope를 발급·검증 양쪽에서 엄격하게 적용
- 스킴을 포함한 Origin 검증과 명시적인 CORS 정책
- CSRF 방어와 카카오 OAuth `state` 검증
- 로그인·도메인 등록·토큰 발급·조회 API의 속도 제한과 할당량
- 지속형 세션 저장소, 안전한 프록시·쿠키·만료 설정
- 입력 정규화·검증, 페이지네이션과 최소 응답 필드
- 카카오 프로필·게시물·오류·인증 정보의 개인정보 최소화와 로그 마스킹
- `sync()`를 대체하는 명시적 DB 마이그레이션과 시작 순서 보장
- 세션·도메인·JWT·CORS·DB·오류 응답을 포함하는 라우트·통합 테스트

위 조건을 충족하고 운영 NodeBird와 분리된 샘플 DB·계정·카카오 앱에서 읽기 전용 API를 구성한 뒤, 별도의 최소 권한 클라이언트로 NodeCat 연동 데모를 검증하고 Unlicense와 `UNLICENSED`의 라이선스 충돌까지 해소하면 **B. 웹앱 전환 후보**로 재분류할 수 있습니다.
